...
@ARCH_REGISTRY.register()
class latent_encoder_gelu_fft(nn.Module):

    def __init__(self, in_chans=6, embed_dim=64, block_num=4, stage=1, group=4, patch_expansion=0.5, channel_expansion=4):
        super(latent_encoder_gelu_fft, self).__init__()

        #assert in_chans == int(in_chans//stage), "in chanel size is wrong"
    ...
    def forward(self, inp_img, gt=None):
        if gt is not None:
            ...
            inp_img_fft = torch.fft.fft2(inp_img, norm='ortho')
            gt_fft_real = gt_fft.real
            gt_fft_imag = gt_fft.imag
            inp_img_fft_real = inp_img_fft.real
            inp_img_fft_imag = inp_img_fft.imag
            x = torch.cat([gt_fft_real, gt_fft_imag, inp_img_fft_real, inp_img_fft_imag], dim=1)
        else:

            inp_img_fft = torch.fft.fft2(inp_img, norm='ortho')
            inp_img_fft_real = inp_img_fft.real
            inp_img_fft_imag = inp_img_fft.imag
            x = torch.cat([inp_img_fft_real, inp_img_fft_imag], dim=1)
